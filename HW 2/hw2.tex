...
\fillinMC{A line of the code is leading to undefined behavior.}\\
\fillinMC{This code has a compiler error on line 6.}\\
\fillinMC{This code will seg-fault on line 4.}\\
\answerMC{None of the other answers is true.}\\
...
\fillinMC{The {\tt getName} function returns a dangling pointer.}\\
...
\answerMC{Double free error.}\\ %To be looked at further
\fillinMC{Memory leak.}\\
\fillinMC{C++ compilation errors.}\\
\fillinMC{Segmentation fault.}\\
\fillinMC{None of the other answers is true.}\\

\subsection*{MISC4 (1.5pts)}
How would you fix the error(s)?\\ %TBD
\fillinMC{Explicitly implement a copy constructor.}\\
\fillinMC{Declare {\tt caffelatte} outside the {\tt if} statement.}\\
...
Which of the following statements complete the code above so that the output is {\tt etnybtcbapa3}? %...wut?
...
\fillinMC{No, because we need to change the {\tt next} field in the previous node.}\\
...
\fillinMC{$O(n)$ for {\tt enqueue} and $O(1)$ for {\tt dequeue}.}\\
...
For each of the elements in the new array with index $i$, you'd assign the element at index \fillinblank{mmmmm} in the old array.
...
\fillinMC{The element at the bottom of {\tt s} is returned.}\\
...
\fillinMC{The elements of {\tt s} are reversed.}\\
...
\fillinMC{{\tt baz} has never been invoked and {\tt curr} points to the second last element.}\\
...
\fillinMC{Doubly circular-linked list with head pointer only.}\\
...
\fillinMC{Insert an item before the head pointer }\\
\fillinMC{Insert an item after the head pointer  }\\
\fillinMC{Delete the item before the head pointer  }\\
\fillinMC{Delete the item after the head pointer}\\
